...
function conPendientesVeredicto(audiencia) {
  ...
  // Solo las columnas ligeras: la tabla no muestra ni conversación ni prompts.
  const corte = conCorteLigero_(h);
...
function conSalud(audiencia) {
  ...
  const hC = cabecera(ZD.HOJA_CONVER);
  const shC = hoja(ZD.HOJA_CONVER);
  if (shC && shC.getLastRow() > 1) {
    ...
    if (cA < 0) {
      ...
      const vacias = shC.getRange(2, cA + 1, n, 1).getValues()
        .filter(f => !String(f[0] || "").trim()).length;
      if (vacias)
  ...
  const shH = hoja(HIST.SHEET_NAME || "Historico");
  if (shH && shH.getLastRow() > 1) {
    const hH = cabecera(HIST.SHEET_NAME || "Historico");
    const iH = x => hH.indexOf(x);
    const nF = shH.getLastRow() - 1;
    const leer = k => k >= 0 ? shH.getRange(2, k + 1, nF, 1).getValues().map(f => f[0])
                             : new Array(nF).fill("");
    if (iH("Audiencia") >= 0 && iH("Auditable") >= 0) {
      const gr = leer(iH("Group Name")), jo = leer(iH("Journey Id")),
            ag = leer(iH("Assignee FullName")), au = leer(iH("Abi/Auto Answer")),
            t1 = leer(iH("ES Output Tags 1st Level v2")),
            t2 = leer(iH("ES Output Tags 2nd Level v2")),
            t3 = leer(iH("ES Output Tags 3rd Level v2")),
            ad = leer(iH("Auditable")), ac = leer(iH("Audiencia"));
      let desajustadas = 0, ganarian = 0;
      for (let k = 0; k < nF; k++) {
        const t = { grupo: String(gr[k] || ""), journey: String(jo[k] || ""),
                    agent: String(ag[k] || ""), auto: String(au[k] || ""),
                    tag1: String(t1[k] || ""), tag2: String(t2[k] || ""),
                    tag3: String(t3[k] || ""), audiencia: String(ac[k] || "").trim() };
        histEvaluarAlcance_(t);
        const antes = audAlcanceHistoricoOk_(ad[k]) ? "SI" : "NO";
  ...
  if (shC && shC.getLastRow() > 1) {
    ...
    if (cAu >= 0 && cTk2 >= 0) {
      const nC = shC.getLastRow() - 1;
      const idsC = shC.getRange(2, cTk2 + 1, nC, 1).getValues();
      const auC  = shC.getRange(2, cAu + 1, nC, 1).getValues();
      const vivos = {};
      conAuditables_(ss, "").forEach(t => { vivos[t] = true; });
      let bloqueados = 0;
      const ejemplos = [];
      for (let k = 0; k < nC; k++) {
        const t = zdClaveTicket_(idsC[k][0]);
        if (!t) continue;
        const v = String(auC[k][0] || "").trim();
  ...
  if (shC && shC.getLastRow() > 1) {
    ...
    if (cBand >= 0 && cTk >= 0) {
      const n = shC.getLastRow() - 1;
      const ids = shC.getRange(2, cTk + 1, n, 1).getValues();
      const bands = shC.getRange(2, cBand + 1, n, 1).getValues();
      const truncados = [];
      bands.forEach((f, i) => {
        if (String(f[0] || "").indexOf("conversacion_truncada") >= 0)
          truncados.push(zdClaveTicket_(ids[i][0]));
      });
      if (truncados.length)
